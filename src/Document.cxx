...
LineInlayHints *Document::InlayHints() const noexcept {
	return dynamic_cast<LineInlayHints *>(perLineData[ldInlayHints].get());
}
...
int Document::SetInlayHint(Sci::Line line, Sci::Position position, const char* text, int style, bool paddingLeft, bool paddingRight, int handle) {
	...
	if (result >= 0) {
		...
		const DocModification mh(SC_MOD_CHANGEINLAYHINT, LineStart(notifyLine), 0, 0, nullptr, notifyLine);
		NotifyModified(mh);
	}
	return result;
}
...
void Document::InlayHintRemove(int hintHandle) {
	...
	if (found) {
		const DocModification mh(SC_MOD_CHANGEINLAYHINT, LineStart(line), 0, 0, nullptr, line);
		NotifyModified(mh);
	}
}
